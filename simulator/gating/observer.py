...
principle (`axioms.no_privilege_consequences()`), not extra inputs:
  ...
  2. **Strong isotropy (arc-transitivity)** — the walker's causal state is a
     ...
     **among the |E| = 3 ℝ³ crystal nets, srs is the unique one** (R-9 closure).
     This **collapses Axis B onto srs**. Front-end: `walker_dynamics_derivation.md`
     Step 4b + `g_girth_derivation.md` Step 2; chain object:
     `menus.crystal_nets.framework_substrate_selection()`.
...
def condition_crystal_net_menu(crystal_net_menu: list) -> list:
    """Collapse the Axis-B crystal-net menu onto the arc-transitive one (srs).

    Among the k* = 3 ℝ³ crystal nets, srs is the unique arc-transitive one
    (Sunada 2012; R-9 closure). `crystal_net_menu` entries are
    `menus.crystal_nets.CrystalNet`; returns the ones with `arc_transitive` and
    `coordination == k*` (= [srs]).
    """
...
def conditioned_substrate() -> dict:
    ...
    return {
        ...
        'axis_B_conditioned': [n.name for n in nets],   # → ['srs']

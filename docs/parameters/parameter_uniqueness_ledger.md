...
- *Gap.* A5(b) is not derived from A1 + P1' alone — prior gap unchanged. **2026-06-22: the Level-2-vs-Level-3 SELECTION is now GROUNDED in the derived local-vs-cumulative recurrence — V_cb touches the heavy gen-3 (real Perron/cumulative mode) ⇒ Level-3 walk-sum (`../theorems/theorem_A5b_level_selection_from_recurrence_2026-06-22.md`); the residual narrows to A5 itself (species labeling), not the level-prescription.** R-9 srs-z gap CLOSED 2026-05-02; tiers (b)/(c) remain Phase 1d asserted (empirically inverse-validated).
...
- *Gap.* A5(b) Level 3 sub-class identification (prior gap). **2026-06-22: the level-SELECTION is GROUNDED — V_us is light-only (no heavy/Perron generation touched) ⇒ Level-2 local uniform-density, forced by the local-vs-cumulative recurrence (`../theorems/theorem_A5b_level_selection_from_recurrence_2026-06-22.md`); residual narrows to A5 (species labeling). Reconcile the V_us label: `V_us.py` "Level 2" vs the §4 table "B (special)" → "Level-2 / local uniform-density."** R-9 srs-z structural gap CLOSED 2026-05-02 EOD+8.
...
- **2026-07-01 — the β-coefficient DERIVATION is now TOP-DOWN (supersedes the 2026-05-14 data-inversion as the primary route).** `derivation_topdown/bridge/the_run.py` `read_gauge_running` computes b₁,b₂,b₃ = {33/5, 1, −3} EXACTLY from the forced Cl(6)-Fock 2HDM content + the COMPUTED 4D time-completion `(1/3)ΣT_f+(2/3)ΣT_H+(2/3)C₂(G)` — NO PDG input (R-19 DE-ESCALATION, `../audits/registers/structural_residue_register.md`). The 2026-05-14 inversion `b_i=(2π/ln)(1/α_i(M_Z)−24)` is now the DATA-SIDE CROSS-CHECK, not the derivation (it uses PDG α_i(M_Z); the values are exact top-down). The one open piece = ζ_{D₄}(0) (the completion's forced-ness; `../incomplete_equations_todo.md` §5); GATES NO VALUE.
...
- *Status.* **UNIQUE-THEOREM-GRADE-CONDITIONAL on (β-coefficients DERIVED TOP-DOWN [exact, the_run.py 4D-completion; forced-ness = ζ_{D₄}(0)], α_GUT dark correction [theorem-grade-cond Routes H+C], the adopted N_hub) jointly.**  Cluster numerical match: α_EM(M_Z), g_1, g_2, sin²θ_W within 0.04% of PDG (most within 0.02%); 1-3σ_PDG range.  Clause 8 PASS for g_1; near-PASS for others.
- *Conditional on.* Row P62 M_unif; **`the_run.py` `read_gauge_running`** (β-coefficient values DERIVED TOP-DOWN — exact, PDG-free; forced-ness of the 4D-completion = ζ_{D₄}(0), research-level), with **theorem_beta_coefficients_derived.md** now the PDG data-side cross-check; **theorem_alpha_GUT_dark_correction.md** (α_GUT dark correction via Routes H+C; 2026-05-15); **ADOPTED-MSSM-Sb particle-interpretation residue** (literal SUSY-partner identification still adopted); **ADOPTED-N_HUB** (value pinned via the measured G_F).
- *Gap.* (1) Literal MSSM particle interpretation still adopted. (2) The top-down derivation (the_run.py completion) is PDG-free/exact; its open piece is ζ_{D₄}(0) (the completion's forced-ness). [The old data-inversion, now a cross-check, uses external PDG α_i(M_Z) — that cap applies to the cross-check, not the derivation.] (3) Q-space Feshbach coupling Part B (= α_1_bare) currently ADOPTED; α_GUT dark correction inherits this.
...
- *Conditional on.* Row P63 (α_EM cluster), M_unif Row P62, v_higgs FSS family, **`the_run.py` `read_gauge_running`** (β top-down, exact/PDG-free; **theorem_beta_coefficients_derived.md** = PDG cross-check), **theorem_alpha_GUT_dark_correction.md**, **ADOPTED-MSSM-Sb particle-interpretation residue**, **ADOPTED-N_HUB**.
...
All 6 inherit Row P63's reframing.  The β-coefficient dependency that
was previously "ADOPTED-MSSM-Sb" is now the top-down 4D-completion (`the_run.py`
`read_gauge_running`, exact/PDG-free; `theorem_beta_coefficients_derived.md` = PDG cross-check)
...
**DERIVED TOP-DOWN** (exact, no PDG input) via the run's 4D time-completion
(`the_run.py` `read_gauge_running`); the PDG-inversion `theorem_beta_coefficients_derived.md`
(one-line algebra from upstream + one-loop RG + PDG α_i(M_Z) [external]) is now the
data-side cross-check.  The **literal MSSM particle interpretation** (sfermions,
...
- AFTER  (2026-07-01): UNIQUE-THEOREM-GRADE-CONDITIONAL on (β DERIVED TOP-DOWN [the_run.py 4D-completion, exact; forced-ness = ζ_{D₄}(0)], N_HUB) jointly — theorem_beta_coefficients_derived = PDG cross-check
...
- *Conditional on.* M_Z Row P64 (incl. its Phase C/C.1 δρ closure); sin²θ_W(M_Z) Row P65; v_higgs FSS family; **`the_run.py` `read_gauge_running`** (β top-down, exact/PDG-free; **theorem_beta_coefficients_derived.md** = PDG cross-check), **theorem_alpha_GUT_dark_correction.md**; **standard Type-3 EW tree tier (m_W=M_Z cosθ_W, W-field normalization — same tier throughout the EW sector)**; **ADOPTED-MSSM-Sb particle-interpretation residue**; **ADOPTED-N_HUB**.  (c=1/2 transition-count conditional REMOVED — closed by Phase C.1.)
...
2. **The deepest remaining structural input at the parameter level is A5(b)'s sub-class identification.** Currently per `theorem_A5b_level_prescription.md`, the criterion is "derivation-structure (how α₁ enters the formula)." Closure of A5(b) from upstream rows would replace this Level 1 prescription input with derivation; that's research-level work. Until then, parameter rows are CONDITIONAL on A5(b) Level + sub-class.
   **UPDATE 2026-06-22 — the level-SELECTION is now GROUNDED (no longer structure-only).** `../theorems/theorem_A5b_level_selection_from_recurrence_2026-06-22.md` derives the Level-2-vs-Level-3 selection from the already-derived local-vs-cumulative recurrence law: a transition is **Level-3 walk-sum iff it touches the heavy gen-3 (which rides the real Perron / CUMULATIVE recurrence mode); light-only ⇒ Level-2 local density** (verified V_us=L2, V_cb/V_ub=L3). So "how α₁ enters" is the *consequence* of touching the cumulative mode, not an independent criterion. The flavor level-structure (forms + winding-lengths + selection) is therefore **forced GIVEN A5**; the lone remaining input narrows to **A5 itself** (the species/Hamming-weight labeling — derive-from-A1 or irreducible anchor). Parameter rows are now CONDITIONAL on A5 (species labeling), not on an undischarged A5(b) sub-class prescription.
...
| P63 | α_EM(M_Z) ≈ 1/127.93 | UNIQUE-THEOREM-GRADE-CONDITIONAL via 5-stage gauge-coupling closure (2026-05-04 EOD+1, `../theorems/theorem_gauge_unification_RG_closure.md` + `proofs/foundations/gauge_unification_full_RG_closure.py`). Inheritance from P40 (α_GUT) + `the_run.py` `read_gauge_running` (β-coefficients DERIVED TOP-DOWN, exact/PDG-free; `theorem_beta_coefficients_derived.md` = PDG cross-check; forced-ness = ζ_{D₄}(0)) + Row 4 inheritance for k*-axis. Clause 8 PASS +1.01σ_PDG. |

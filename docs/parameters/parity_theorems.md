...
stack" and is the public-repo equivalent of R_theorem.md for the
R = 228/7 result.
...
parity-violating observables with zero free parameters.
    ...
    THEOREM-GRADE-STRUCTURAL (downgraded 2026-05-16 from UNIQUE-THEOREM-GRADE):
                beta = sin(arg h) * alpha_EM(M_Z) = 0.354 deg    (dark correction;
                framework alpha_EM, with Delta-alpha Clause-9-blocked named gap)

### Numerical matches

    A = 1/15 = 0.06667       obs: 0.07 +/- 0.02          -0.17 sigma
    beta = 0.354 deg         obs: 0.342 +/- 0.094 deg    +0.13 sigma
...
pattern predicts
    ...
    beta = sin(arg h) * alpha_EM(M_Z) = 0.354 deg

using the framework's RG-run alpha_EM(M_Z) ~ 1/127.93 (per predictions/
alpha_EM.py, THEOREM-GRADE-CONDITIONAL post 2026-05-04 EOD+1 5-stage
gauge-coupling closure).

Match to Eskilt 2022 (beta_obs = 0.342 +/- 0.094 deg): +0.13 sigma.

### Grade: THEOREM-GRADE-STRUCTURAL (downgraded 2026-05-16)

This derivation has been downgraded from UNIQUE-THEOREM-GRADE to
THEOREM-GRADE-STRUCTURAL because the earlier numerical value 0.331 deg
used observed alpha_EM = 1/137.036, which is NOT framework-derived without
the Delta-alpha low-energy hadronic threshold (Clause-9-blocked /
Move-1-out-of-scope). Rewiring to framework alpha_EM(M_Z) at M_Z gives
0.354 deg as the honest framework value; the FORM (c=1*sin(arg h) =
sqrt(5/8)) remains theorem-grade.

The amplitude-class dark-correction rule (linear in sin(arg h) via
alpha_EM) is empirically supported by the neutrino sector and is the
amplitude analog of the tan^2(arg h) mass-squared rule. Theorem 4
forces beta to be dynamical (no topological source available), and the
amplitude-linear form is the only dimensionally consistent chirality
invariant at leading order in alpha_EM. The remaining named gap is the
substrate analog of the Delta-alpha low-energy threshold that bridges
alpha_EM(0) to alpha_EM(M_Z) — Clause-9-blocked per parameter_linter.md
Clause 9 (Type-3 SM-import pi-audit, K-rationality bright-line).
...
The parity-violation sector now has:
    ...
    - Theorem 2 (A = 1/15)                   verified (-0.17 sigma)
    - Theorem 3 (B(P) doubly-degenerate h)   verified
    - Theorem 4 (c_1 = 0 on slices)          verified
    - beta = sin(arg h) * alpha_EM(M_Z)      THEOREM-GRADE-STRUCTURAL (+0.13 sigma; framework alpha_EM)

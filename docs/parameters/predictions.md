...
> **Status (2026-05-27, post-SUSY-load-bearing audit):** the framework's substrate-derived matter content is 3 PS generations + 2 Higgs doublets, with NO superpartners (Cl(6) Fock all-fermionic per Path-E recheck 2026-05-12; A1 thermal-apparatus closure 2026-05-27). The MSSM β-coefficient values (33/5, 1, −3) the framework predicts are derived TOP-DOWN by the run's 4D time-completion (`derivation_topdown/bridge/the_run.py` `read_gauge_running` — exact, no PDG input; the PDG-inversion [`theorem_beta_coefficients_derived.md`](../theorems/theorem_beta_coefficients_derived.md) is now the data-side cross-check), NOT by counting literal sparticles. The gap between substrate-derived 2HDM β (b_2 = −3) and observation-imposed MSSM β (b_2 = +1) is precisely characterized as Δb_2 = +4 ([R-19](../audits/registers/structural_residue_register.md)).
>

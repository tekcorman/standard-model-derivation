...
3. **SUSY partner Hilbert spaces (RESOLVED 2026-05-27 — open question retired).** The framework does NOT include literal SUSY partners in its substrate-derived particle catalog. The substrate's matter content is 3 PS generations + 2 Higgs doublets, all-fermionic via Cl(6) Fock per Path-E recheck. The MSSM β-coefficient values are derived TOP-DOWN by the run's 4D time-completion (`the_run.py` `read_gauge_running`, exact/PDG-free; the PDG-inversion is now the data-side cross-check), not by literal-particle counting. Sparticle-spectrum entries in `../parameters/predictions.md` §SUSY Spectrum are honest-conditional only. See [R-19](../audits/registers/structural_residue_register.md) + SUSY-load-bearing audit.

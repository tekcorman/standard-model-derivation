...
- `../theorems/theorem_car_local_jordan_wigner.md` (A4 → local theorem via Jordan-Wigner; global ordering-REQUIREMENT dissolved 2026-06-11 by the Phase 5.3 link-dressing bridge — ordering is gauge within a flux sector; conditional residue per the theorem's revised scope limit)
...
- **A5(b) — Coupling clause.** The total branch-measure probability of all above-waterline NB walk representations of the process is identified with the physical coupling strength. Under A2's waterline, the retained walk class for a process with n_fixed pinned endpoint causal states consists of all girth-cycle winding classes {n × (g−n_fixed) steps : n = 1, 2, 3, …}, each of which saves Ω(n) bits over uncompressed description. The coupling is the sum over all retained windings:
  ...
  **Criterion for Case selection (derivation-structure, not observation).** If the coupling's derivation expresses it as a closed-form function of α₁ and other graph invariants, Case (A). If the derivation is a sum Σ_n α₁^{f(n)} with n indexing distinct walk representations, Case (B). If Moore-equivalent uniform, Case (B'). The classification is determined by the STRUCTURAL ROLE of α₁ in the formula — the question of which level it appears at — independent of observational match. See `../theorems/theorem_A5b_level_prescription.md` for the full theorem and classification audit. **Update 2026-06-22:** the level-*selection* itself (Level-2 single-density vs Level-3 walk-sum) is now DERIVED, not merely prescribed — `../theorems/theorem_A5b_level_selection_from_recurrence_2026-06-22.md` forces it from the derived ∂_N local-vs-cumulative recurrence law ("a transition's level = the recurrence-type of the heaviest generation it touches"; no adoption). So under A5(b) the *identification* "MDL probability = coupling" remains the semantic content, but *which* MDL form applies is a forced read, not a choice. (The within-Level-3 sub-class classifier ADOPTED-A5b-Sub3 is a separate, still-active adoption — graduation retracted 2026-04-29; the 2026-06-22 result does not close it.)
...
- `../theorems/theorem_car_local_jordan_wigner.md` — A4 locally derived (Session 11) via Jordan-Wigner. Global CAR: the total-ordering REQUIREMENT (B1 ordering) was dissolved 2026-06-11 (Phase 5.3 B3 bridge + b3b lattice gates + ultracode panel, verdict PARTIAL): link-dressed CAR with no node ordering, dressing = gauge within a flux sector; conditional residue (flux-sector classification; even-subalgebra/parity-superselection reading covered by the retained A4 selection bit) per the theorem's revised scope limit. No current prediction-DAG file consumes global CAR (W1 audit, 2026-04-26; re-confirmed by the 5.3 panel).

...
### (B′) Feshbach channel read at the PERRON channel — heavy-quark absolute anchors (added 2026-06-25, CONDITIONAL)

The Family (B) channel read $\Sigma(h) = \alpha_1/h$ (the leading **first-girth-return** of the visible NB resolvent $G_{\rm NB}=(I-uB)^{-1}$ at $u=\alpha_1$; mechanism settled in an internal working note §5 — the real Dyson insertion $\Sigma=\alpha_1\times(1/h)$, reproducing the shell $\sqrt5/4$ / Perron $1/2$ / $L{=}0\to1/4$ to 1e-12; NOT a Q-space contour, the Schur$=0$ is consistent-but-irrelevant) applies not only at the SHELL channel ($h=(\sqrt3+i\sqrt5)/2$, parity-odd $\mathrm{Im}(1/h)=\sqrt5/4$, Family B above, used for $m_\nu$) but at the **PERRON channel** ($h_P=k_*-1=2$, REAL) for the **heavy-quark ABSOLUTE ANCHORS**:

$$m_q^{\rm phys} = m_q^{\rm bare}\times\Bigl(1 - \frac{\alpha_1}{h_P^{\,p}}\Bigr),\qquad p = \begin{cases}2 & L=0\ (\text{saturation, } y_t{=}1)\\ 1 & L>0\end{cases}$$

- **L-power rule** (the read's tensor rank): $L=0$ (the $y_t{=}1$ saturation ceiling — no propagating line, only the normalization $|a_0|^2$ is dressable) → 2-point → $p=2$; $L>0$ (propagating amplitude) → 1-point → $p=1$. Fixed by the single bit "is $L=0$?".
- **Channel** = Perron $h_P=2$ (the heavy generation rides the Perron, vs the shell for $\nu$) — set by the sector's $\mathrm{chir}$.
- **Sign** = negative (the Q-space drains the channel).

| anchor | $L$ | $p$ | factor | bare → predicted | obs | σ_PDG: was → now |
|---|---|---|---|---|---|---|
| $m_b$ (gen-3 down) | 10 | 1 | $1-\alpha_1/2$ | $4.270\to4.187$ | 4.18 | $+2.99\to+0.22$ |
| $m_t$ (up saturation) | 0 | 2 | $1-\alpha_1/4$ | $174.10\to172.41$ | 172.69 | $+4.71\to-0.95$ |

**Grade (CORRECTED 2026-06-28 — see `theorem_dark_self_energy_unified_2026-06-28.md`): MAGNITUDE-FORCED, SIGN-OPEN.**
- **Forced:** the magnitude $\alpha_1/h_P^{\,p}$ (cross-checked to 1e-12 on the lepton/ν coefficients), the
  channel (Perron, quark-sector Ihara-Bass root), the power (L-rule), and the **coefficient c=1** — the d/b
  IS the Perron channel mode (the "saturated channel", the FULL residue), NOT the single-edge $1/12$.
  ⚠ The single-edge $1/(2|E|)=1/12$ is the *vertex* projection ($c_F$, Family D) and the gauge-singlet
  projection ($c_S$, $\delta_r$) — a DIFFERENT object; the 2026-05-15/18 framing that applied $1/12$ here
  was stale (lagged the channel-read/full-residue finding).
- **SIGN — EMPIRICALLY forced DOWN, NOT formally forced (corrected 2026-06-29).** The downward factor
  $(1-\alpha_1/h_P^{\,p})$ is **empirically** established: m_b prefers DOWN (+0.22σ vs +5.8σ for UP), the leptons/ν
  cross-check the coefficient to 1e-12, and the sign is consistent across the sector. A clean formal lemma for the
  *channel-read* sign does NOT close: "mass = dynamical recurrence" admits THREE readings of the first-girth-return —
  literal (h/k)^L at fixed L → no-change; rate/velocity → DOWN; return amplitude → UP (verified symbolically). The
  generic "mass=recurrence" reading admits all three; the framework's foundational mass=recurrence-RATE definition
  picks the rate reading → DOWN. Full status: `theorem_dark_self_energy_unified_2026-06-28.md` §3. So the channel-read
  darks are **magnitude THEOREM-GRADE, sign DERIVED conditional on the rate foundation** (+ empirically cross-checked +
  consistent) — settled DOWN; the standalone formal CAS lemma (rate-reading selection) is the named open piece.
- The *vertex* sign (y_τ $c_F$) IS forced DOWN (separate closed fermion loop, −1, Peskin §4.8) — but that does
  NOT extend to the channel reads (own-self-energies, not separate loops).

**Distinct from (D) and the koide ratio — this does NOT contradict the "Family D non-applicability to quark sector" audit (2026-05-15, §3 (D)).** That audit concerns (i) Family (D), the per-leg VERTEX $\alpha_1^2$ dark (blocked for quarks via Need-D-3 — no tree-level quark Yukawa to correct), and (ii) the koide_quark_ratio (P37) residual (a Cl(6) Fock-expansion structural artifact of the RATIO). (B′) is a different object: the Feshbach CHANNEL read on the heavy-quark ABSOLUTE ANCHOR, neither a vertex dark nor a ratio. The audit is silent on it; (B′) extends the taxonomy, it does not overturn the audit.

# β-coefficients — the PDG-consistency cross-check of the top-down b = (33/5, 1, −3)

> ⚠ **SUPERSEDED AS THE PRIMARY DERIVATION (2026-07-01).** The genuine top-down
> derivation of the EXACT b = (33/5, 1, −3) is the run's 4D time-completion —
> `derivation_topdown/bridge/the_run.py` `read_gauge_running`: the 2HDM β
> (computed from the forced Cl(6)-Fock content) plus the COMPUTED completion
> `(1/3)ΣT_f + (2/3)ΣT_H + (2/3)C₂(G)` reproduces b₁,b₂,b₃ = {33/5, 1, −3}
> **EXACTLY, with NO PDG input** (verified; R-19 DE-ESCALATION in
> `structural_residue_register`; `docs/incomplete_equations_todo.md` §5).
> **THIS doc is retained as the independent DATA-SIDE CROSS-CHECK:** it inverts
> one-loop RG from the MEASURED α_i(M_Z) and recovers b ≈ (33/5, 1, −3) to
> ~1-6%, confirming the top-down values are consistent with the observed
> running. It is NOT the derivation — it USES PDG α_i(M_Z) as input, so per the
> top-down law it cannot be the *source* of b_i. (Whether the top-down
> completion is FORCED — vs reproduces — is the open piece ζ_{D₄}(0), todo §5.)

**Status:** DATA-SIDE CONSISTENCY CROSS-CHECK (uses external PDG α_i(M_Z)); the
primary derivation is the top-down completion (the_run.py), exact and PDG-free.

**One-line statement.** Inverting one-loop RG from the framework's theorem-grade
upstream boundary (α_GUT⁻¹ = 24, sin²θ_W = 3/8 at M_unif) down to the MEASURED
α_i(M_Z) recovers b_i ≈ (33/5, 1, −3) within ~1-6% — a consistency check on the
exact values the top-down 4D-completion derives.

**Why this matters.** It is an INDEPENDENT, data-side confirmation: the exact
b = (33/5, 1, −3) that the run's 4D-completion computes top-down (no PDG input)
are the same values one recovers by inverting the observed running. Two routes,
one answer — the top-down completion is the derivation; this inversion is the
data-side cross-check. Only the literal particle realization (sfermions,
gauginos, Higgsinos) remains a separate open question (§6).
...
**Role:** DATA-SIDE CONSISTENCY CROSS-CHECK — NOT the primary derivation.

**Primary derivation of b = (33/5, 1, −3):** the top-down 4D time-completion,
`derivation_topdown/bridge/the_run.py` `read_gauge_running` — exact, no PDG input
(todo §5; R-19 de-escalation in `structural_residue_register`).

**This doc's external inputs:** PDG α_i(M_Z) values (three) — which is precisely why
it is a cross-check, not the derivation.

**Cross-check match:** the inversion recovers b to ~1-6% (b_2 +6.22%); observable-level
0.6-2.8% — consistent with the exact top-down values.

**Supersedes:** its own earlier "this IS the derivation of the β-coefficient piece of
ADOPTED-MSSM-Sb" framing — the exact values are derived top-down; this is the data check.

**Does not touch:** the literal particle-content piece of ADOPTED-MSSM-Sb (whether
sfermions/gauginos physically realize the values), which remains a separate open question.

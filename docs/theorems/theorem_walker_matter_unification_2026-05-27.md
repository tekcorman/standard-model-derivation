...
These two 48-counts are **structurally identical**: the substrate's walker-class structure at the Ramanujan saddles is, mode for mode, the framework's SM matter content. Specifically:

> **For every framework Weyl-spinor SM matter state X_α (α = 1, …, 48), there is a unique Hashimoto eigenmode \|w_α⟩ ∈ ⨁_{k ∈ {Γ,P,N,H}} ℂ^{12} such that X_α is identified with \|w_α⟩ via the V_Ram ≅ Cl(6) Fock isomorphism extended across all four saddles, with chirality and generation labels inherited from the C_3 isotypic + γ_7 grading + saddle-arg-sign of \|w_α⟩.**
...
This is **the same B_NB substrate resolvent G_NB = (I − u·B_NB(srs))⁻¹** reading out 12 distinct observables (per `state_of_the_lepton_pmns_over_determination_2026-05-23.md`).

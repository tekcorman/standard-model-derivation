...
**Scope limit (revised 2026-06-11, Phase 5.3 B3 + ultracode panel, verdict PARTIAL):** this is LOCAL A4 at each node. Global CAR across distinct nodes does NOT require a total node ordering: per-edge link operators u_e = b_u b_v built from the per-node Cl(6) dress the matter Majoranas along any spanning tree, and the dressed set satisfies full CAR — machine-verified at zero on srs clusters (`proofs/foundations/phase5_3_b3_bridge_global_car_2026-06-11.py` D0–D5; lattice-extension gates `phase5_3_b3b_lattice_extension_2026-06-11.py` E1–E4: middle-root automorphism, one-end relabeling, the degree-3 branched star, and a cyclic cluster with two spanning trees). Dressing choices (tree, root, b/c labeling, per-χ sign) are CAR-algebra automorphisms; on cyclic clusters a spanning-tree change shifts charged bilinears by exactly a central ring (Wilson-loop) operator W — i.e. ordering/dressing is gauge WITHIN A FLUX SECTOR, not unconditionally. CONDITIONAL: (a) finite clusters are machine-gated; the lattice-wide statement rests on the parity-counting lemma (b3b docstring), filed with its gated legs; (b) the flux-sector residue is physical and partially unclassified (B2's stable GF(2) deficit-2 invariants at the Γ anomaly; odd-torus non-contractible loop operators square to −1, a non-Z₂ sector — named follow-ups); (c) the "physical = even/dressing-invariant algebra" reading (fermion-parity superselection) is not yet derived natively — covered by the retained A4 1.0-bit selection row; (d) physical identification of the dressed operators with SM fermions remains downstream of A5 (unchanged). The prior sentence here ("requires a consistent total ordering of all nodes, the B1 ordering workstream") was too strong as a requirement claim and is superseded.

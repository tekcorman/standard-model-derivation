...
**(A) β-coefficient values (b_1, b_2, b_3) = (33/5, 1, −3).**  GRADUATED to **mathematically complete** derivation via `docs/theorems/theorem_beta_coefficients_derived.md` (2026-05-14).  The derivation chain is:
- α_GUT⁻¹ = 24 at M_unif (theorem-grade upstream)
- sin²θ_W = 3/8 at M_unif (theorem-grade upstream)
- M_unif scale (theorem-grade-conditional upstream)
- One-loop RG running (textbook Type 3)
- PDG α_i(M_Z) values [external observational input]

Algebraic inversion gives b_i = (2π/ln(M_unif/M_Z)) × (1/α_i(M_Z) − 24).  Numerical evaluation produces (33/5, 1, −3) within ~1-6%. Observable-level deviations 0.6-2.8% are reported against σ_PDG only (FAIL Clause 8 against σ_PDG alone).

The grade is *mathematically complete* (not full theorem) because of the three [external] PDG inputs — same treatment as M_Z itself in the framework.
...
> UNIQUE-THEOREM-GRADE-CONDITIONAL on (β-coefficients derived [mathematically complete, theorem_beta_coefficients_derived.md], ADOPTED-N_HUB) jointly.

...
Across **123 tracked targets**, counted in **honest σ_PDG** (parameter-linter Clause-8, σ_PDG only — no σ_theory widening; the 2026-06-22 baseline split is in [`docs/parameters/honest_sigma_count_2026-06-22.md`](docs/parameters/honest_sigma_count_2026-06-22.md); counts below carry the 2026-06-25 dark-correction shipment forward):

- **42 match within 1σ_PDG** — genuine numerical closures (the 2026-06-22 count of 39, plus **m_t (−0.95σ), m_b (+0.22σ), g_2 (−0.18σ)**, which closed 2026-06-25 when the forced dark self-energy Σ = α₁/h and the scheme-consistent g_2 target shipped): all CKM except the two unitarity-tension entries, the PMNS angles + δ_CP, all six quark masses, m_τ, η_B, β-birefringence, the gauge couplings, the Higgs sector.
...
- **2 open ppm-scale misses** (m_e −70.3 ppm, m_μ −60.5 ppm relative): the un-derived O(α₁³) subleading per-rep correction — a logged open equation ([`docs/incomplete_equations_todo.md`](docs/incomplete_equations_todo.md) §1). The measurements are sub-ppb, so σ_PDG is unreachable either way — but the miss is a miss, and it stays open.
...
| **This framework** | **42 matched within 1σ_PDG + ~19 forced-exact; 12 cross-validated via a single resolvent (9 open gaps, channel-structured & understood)** | **One graph (srs, the dominant MDL-waterline survivor); the non-backtracking resolvent G_NB on it — a read of D = B(srs⊗srs-z) ⊗ ∂_N** |

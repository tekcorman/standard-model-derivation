...
## Coverage note (added 2026-05-26)

This file documents 16 cleanly-negative failure modes (the numbered
entries above), which reference 9 distinct scripts via `Script:`
pointers. The `explorations/` directory contains 25 scripts total; the
16 currently-unindexed scripts are *not* negative results but
exploration scripts whose findings were adopted into live `proofs/`
and `predictions/` derivations (or remain orphaned exploration the
framework has not classified yet). The 2026-05-26 consolidation audit

flagged this gap; per-script classification of the unindexed 16 has
been deferred to a future pass because it requires individual
inspection rather than a mechanical rule. The unindexed scripts are:
`bp_h_eigenspace_c3`, `fermion_mass_probe`, `fermion_mass_probe_refine`,
`srs_alpha21_hashimoto_phase`, `srs_alpha21_mr_derivation`,
`srs_delta_squared_from_p`, `srs_greens_decay`, `srs_holonomy_phases`,
`srs_majorana_md_principled`, `srs_majorana_phases`, `srs_mass_scale_from_p`,
`srs_mass_scale_proof`, `srs_pmns_from_bloch`, `srs_pmns_koide_kpoint`,
`srs_r_investigation`, `srs_theta12_derivation`. Anyone classifying
them: cite the destination in either this file (if negative) or in the
appropriate `predictions/*_derivation.md` (if adopted).

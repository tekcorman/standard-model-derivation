...
SUPERSEDED 2026-06-16 for the SCALE PLACEMENT (Row P46 reconciliation):
  This file places y_b = GJ·y_τ at M_GUT and runs DOWN, giving tan β ≈ 44.73.
  But the framework's scale-assignment rule (theorem_walker_length_MDL_
  waterline §11) says the Type-III (τ) and Type-IV (b) Yukawas are LOW-scale
  values (y_τ=0.007226 reproduces the physical m_τ=v·y_τ=1.777 GeV at low
  scale). The framework-CONSISTENT computation places them at M_Z and runs UP
  — predictions/tan_beta.py, giving tan β ≈ 60.07. Use that value; this file's
  44.73 is a GUT-placement artifact, retained for provenance. (tan β is
  BNDY-EXT either way — external MSSM RGE, not load-bearing for m_t/m_b; the
  invariant framework content is GJ = y_b/y_τ = k* = 3.)

#!/usr/bin/env python3
"""
---
derives: m_u
inputs:
  - m_t
  - alpha_1
  - h_walker_eigenvalue
  - k_star
script_version: 1.0.0
doc: docs/parameters/target_parameters.md
doc_section: quark Yukawa sector / up-type Koide triplet (Stage 1)
doc_version_required: 0.0.1
mechanism: structural
rigor_status: rigor_route_specified
---

m_u from the up-type quark Koide triplet (n=2), anchored to m_t.
Same sector machinery as m_c_derivation.py; m_u uses f_min instead of
f_mid:

    m_u = m_t * (f_min / f_max)^2

In the up sector the smallest factor f_min is near a cancellation
(~0.01 at delta=2/27, eps^2=3.09), so the m_u prediction is
amplified-sensitive to perturbations in alpha_1_full. This is the
analogue of the m_e residual in the lepton sector. The residual is
cascade, not free-parameter.
"""

import os
import sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from _quark_koide import koide_factors, alpha_1_full_from_inputs, shared_inputs


N_SECTOR = 2  # up-type quarks


def derive(m_t: float,
           alpha_1_bare: float,
           h_real: float,
           h_imag: float,
           k_star: float) -> dict:
    if m_t <= 0:
        raise ValueError(f"m_t must be > 0; got {m_t}")
    alpha_1_full = alpha_1_full_from_inputs(alpha_1_bare, h_real, h_imag)
    koide = koide_factors(N_SECTOR, alpha_1_full, k_star)
    ratio_sq = (koide['f_min'] / koide['f_max']) ** 2
    m_u = m_t * ratio_sq
    return {
        'predicted': m_u,
        'checks': {
            'alpha_1_full': alpha_1_full,
            'eps_sq': koide['eps_sq'],
            'delta': koide['delta'],
            'factors_sorted': koide['factors_sorted'],
            'ratio_sq_mu_over_mt': ratio_sq,
            'm_t': m_t,
        },
    }


def _load_inputs() -> dict:
    shared = shared_inputs()
    return {
        'm_t': shared['m_t'],
        'alpha_1_bare': shared['alpha_1_bare'],
        'h_real': shared['h_real'],
        'h_imag': shared['h_imag'],
        'k_star': shared['k_star'],
    }


def main():
    inputs = _load_inputs()
    result = derive(**inputs)

    print(f"# PREDICT name=m_u value={result['predicted']:.15f}")
    print()
    print("m_u = m_t * (f_min / f_max)^2   [up sector, n=2]")
    print(f"  m_t                  = {inputs['m_t']:.6f}  GeV   (framework anchor)")
    print(f"  alpha_1_full         = {result['checks']['alpha_1_full']:.15f}  (= (5/3)*(2/3)^8)")
    print(f"  eps_2^2              = {result['checks']['eps_sq']:.15f}")
    print(f"  delta_2              = {result['checks']['delta']:.15f}  (= 2/27)")
    print(f"  f_min, f_mid, f_max  = {result['checks']['factors_sorted']}")
    print(f"  (f_min/f_max)^2      = {result['checks']['ratio_sq_mu_over_mt']:.15f}")
    print(f"  m_u                  = {result['predicted']:.15f}  GeV")


if __name__ == '__main__':
    main()

...
# q^3 = 5q - 2 at q = k*-1 = 2. See srs_r_theorem.py and docs/parameters/R_theorem.md.)
...
def part3_yukawa():
    ...
    # │   (3) high-symmetry eigenvalues are {3,√3,√5,1,−3}, not 2±√3;        │
    # │   (4) the van Hove band edges are {±1,±√3,±3}; 1−√3 (adjacency       │
    # │       partner of L_us=2+√3) is 0.27 off ANY band edge.               │
    # │ y_nu=0.7436 from this gives m_ν3 −44% with the STRUCTURAL M_R.       │
    # │                                                                       │
    # │ CORRECT (commit 35e4ed3): the neutrino DIRAC Yukawa is SATURATION    │
    # │ y_ν = y_t = 1 — the Pati–Salam SU(4) ν–top-quark partnership (the    │
    # │ shipped predictions/m_nu3.py already uses y_ν=1 + the N_hub scale;   │
    # │ required y_ν = 0.9957 ≈ 1). The neutrino's DIMENSIONLESS structure   │
    # │ (R_ν=228/7, ν_amp=√7/4, m_ν1=0) is the chir-7 Ihara–Bass band-edge   │
    # │ reading at λ=±1, root (1+i√7)/2 — theorem-grade, unaffected.         │
    # │ The block below is kept for provenance ONLY; do not use y_nu=0.7436. │
    # └─────────────────────────────────────────────────────────────────────┘
    #
    # KEY INSIGHT (RETRACTED): the delocalized Yukawa involves the NB walk
    # survival (2/3) TIMES a spectral correction factor sqrt(L_us/k).
    #
    # L_us = 2 + sqrt(3) was claimed as "spectral radius (inverse spectral
    # gap)" — FALSE (see retraction box above).
    #
    # y_nu = (k-1)/k * sqrt(L_us/k)
    #      = (2/3) * sqrt((2+sqrt(3))/3)
    #      = (2/3) * sqrt(1.2440)
    #      = 0.7436   ← RETRACTED VALUE (use y_ν=1 saturation instead)

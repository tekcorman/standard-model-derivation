...
Framework parameters (all derived from toggle + MDL + Laves graph):
  epsilon = 1/5          (CP asymmetry: 9 chiral + 6 P-symmetric ten-cycles per vertex;
                          chirality verified 2026-05-03 in
                          `proofs/foundations/srs_ten_cycle_chirality_split.py` —
                          the 6 are reflection-symmetric (signed projected area = 0
                          on plane ⊥ [111]), NOT opposite-handed cycles. Asymmetry
                          1/5 = (chiral − P_sym) / total reproduces the predicted
                          value but with corrected mechanism reading.)
...
g = 10                                 # Laves girth
# Cycle decomposition (verified 2026-05-03):
#   n_chiral = 9: cycles with non-zero signed projected area on plane ⊥ [111]
#   n_psym   = 6: P-symmetric cycles (signed area = 0; reflection plane through [111])
# The earlier `n_ccw=9, n_cw=6` labeling was orientation-biased; structurally
# correct reading is n_chiral=9, n_psym=6 per
# an internal working note.
# The asymmetry value 1/5 = (chiral − P_sym)/total is preserved; numerical
# baryogenesis predictions unchanged.
n_chiral, n_psym = 9, 6                # ten-cycles per vertex
epsilon = (n_chiral - n_psym) / (n_chiral + n_psym)  # = 1/5

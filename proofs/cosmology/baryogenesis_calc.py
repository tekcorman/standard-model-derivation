...
# Chirality verified 2026-05-03 (proofs/foundations/srs_ten_cycle_chirality_split.py):
# the 6 cycles are NOT opposite-handed but P-symmetric (signed projected area = 0
# on plane ⊥ [111]; reflection plane through [111] verified at 30°/90°). The
# earlier `n_cw=6, n_ccw=9` labeling conflated chiral with P-symmetric cycles.
# Structural reading: 9 chiral + 6 P-symmetric. Asymmetry value 1/5 preserved.
# Doc: an internal working note
n_psym   = 6                   # P-symmetric (reflection-invariant, was n_cw)
n_chiral = 9                   # chiral (winds around [111], was n_ccw)
epsilon_chiral = (n_chiral - n_psym) / (n_chiral + n_psym)  # = 1/5
...
print(f"  k* = {k}")
print(f"  g = {g}")
print(f"  ε_chiral = ({n_chiral}-{n_psym})/{n_g} = {epsilon_chiral:.4f} = 1/{1/epsilon_chiral:.0f}")

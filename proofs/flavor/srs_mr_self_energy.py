...
This script's conclusion — "STATUS: THEOREM (QED)" for M_R = h^g — is NOT the
framework's current position and should not be cited as such. The 2026-05-12
audit found the per-generation phase factor h_m^g is a bare identification, not
derived (two discharge attempts failed); it is carried as the adoption
ADOPTED-NU-MAJ-PHASE (`docs/audits/registers/adoption_register.md`), and ledger
Row P35 was re-graded down from UNIQUE-THEOREM-GRADE-CONDITIONAL to
STRUCTURAL-DERIVATION-CONDITIONAL. The 1PI=NB-walk and Hashimoto-return content
below stands; the final "M_R = h^g theorem" claim does not.
>>> end notice <<<

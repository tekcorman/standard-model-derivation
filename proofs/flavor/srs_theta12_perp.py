...
def main():
    ...
    # V_us from the framework spectral gap: bare (2/3)^(2+sqrt(3))
    # at the srs P-point. No Feshbach/dark correction at this layer.
    V_us = (2 / 3) ** (2 + np.sqrt(3))
    theta_TBM = np.arctan(1 / np.sqrt(2))  # 35.264°
    theta_C = np.arcsin(V_us)               # Cabibbo angle from bare V_us

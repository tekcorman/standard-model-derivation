#!/usr/bin/env python3
"""
⚠ SUPERSEDED / RETIRED (2026-06-30) — READ FIRST. This file's lepton sector
HARDCODES δ = 2/9 (line ~153) and compares its outputs to observed PDG masses;
that δ=2/9 is an ADOPTED fit, not derived here. It is superseded by the derived
value in predictions/delta_Koide.py (δ=Q(1−Q) from Q=2/3, target-free HM route),
where the honest grade is: the VALUE 2/9 is forced, the generation SPLITTING is
adopted, and the −70 ppm subleading is OPEN. This file is imported by NOTHING
(hyphenated filename → un-importable; git grep confirms zero importers) and is
kept only as historical record. Do NOT cite it as a derivation.
...
def report():
    ...
    print(f"  Lepton Koide phase δ = 2/9 rad (ADOPTED/hardcoded fit — SUPERSEDED; see banner)")
  ...
  [SUPERSEDED verdict — the lepton δ=2/9 here is an adopted hardcode, not a
   derivation; see the file banner. The (4,2,2)/ε²=2 structure is real, but this
   file's "unified framework" claim over-reaches and it feeds nothing live.]

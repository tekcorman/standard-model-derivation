...
WYCKOFF_DATA = {
    ...
    # Source: International Tables for Crystallography Vol. A, sg #214.
    # Cross-referenced against Bilbao Crystallographic Server.
    # Total Wyckoff positions: W=9 (CORRECTED 2026-05-02 — earlier W=5 was
    # an incomplete enumeration that affected dl_srs by log2(9/5)=0.85 bits;
    # symmetric correction applied to lov audit too via lov_dl_audit.py).
    214: {'name': 'I4_132', 'W': 9,
          'positions': {
              '8a':  {'mult': 8,  'free_params': 0, 'coords': '(1/8,1/8,1/8)'},     # site sym .32 (D_3)
              '8b':  {'mult': 8,  'free_params': 0, 'coords': '(7/8,7/8,7/8)'},     # site sym .32 (D_3)
              '12c': {'mult': 12, 'free_params': 0, 'coords': '(1/8,0,1/4)'},       # site sym .2..
              '12d': {'mult': 12, 'free_params': 0, 'coords': '(5/8,0,1/4)'},       # site sym .2..
              '16e': {'mult': 16, 'free_params': 1, 'coords': '(x,x,x)'},           # site sym .3.
              '24f': {'mult': 24, 'free_params': 1, 'coords': '(3/8,y,y+1/4)'},     # site sym .2.  ← lov here
              '24g': {'mult': 24, 'free_params': 1, 'coords': '(1/8,y,-y+1/4)'},    # site sym .2.  ← lou, okw here
              '24h': {'mult': 24, 'free_params': 1, 'coords': '(-x,x+1/4,1/8)'},    # site sym .1.
              '48i': {'mult': 48, 'free_params': 3, 'coords': '(x,y,z)'},           # site sym 1 (general)
          }},
...
def dl_srs():
    """
    SRS net (Laves graph, K4 crystal): I4_132, Wyckoff 8a (fixed position
    (1/8, 1/8, 1/8) with site symmetry .32, no free parameters).
    Vertex-transitive, edge-transitive (UNIQUE strongly isotropic 3D 3-c
    crystal net by Sunada 2012; RCSR's V+E-transitive (1,1) class has 9
    chiral cubic candidates total per `proofs/foundations/rcsr_candidate_sweep.py`).

    Coordinate x = 1/8 is fixed by the Wyckoff position (no free param);
    nearest-neighbor connectivity is determined by the (sg, Wyckoff) choice
    alone.

    NOTE 2026-05-02: Wyckoff component now uses the corrected W=9 for I4_132
    (previously W=5 — `WYCKOFF_DATA[214]` was missing 16e, 24f, 24g, 24h, 48i).
    DL increases by log2(9/5) = 0.85 bits to 18.02. Symmetric impact on srs
    and lov; relative comparisons unchanged.
    """
    bits = {}
    bits['space_group'] = dl_choice(230)               # 7.85
    bits['n_orbits']    = dl_integer(1)                # 1.00
    bits['wyckoff']     = dl_choice(WYCKOFF_DATA[214]['W'])  # 3.17 (8a from W=9)
    bits['coordinates'] = 0.0                          # 8a is fixed; no free params

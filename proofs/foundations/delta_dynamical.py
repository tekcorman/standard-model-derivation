#!/usr/bin/env python3
"""
Dynamical-phase MEASURE SCAN for the Koide phase δ = 2/9 on the Laves graph (srs net).
⚠ NOT A DERIVATION: this file hardcodes `target = 2/9` (~line 1176) and scans ~10
computed measures against it, printing most as explicit misses ("≠ 2/9"); it retains
the one measure (harmonic mean of {4/9,1/9,4/9}) that hits the target. That HM route
is a genuine target-free result (see predictions/delta_Koide_derivation.py), but the
overall "select-the-matching-measure-against-a-hardcoded-target" framing is a scan,
not a forced read. Imported by nothing.
...
def synthesis():
    ...
    print(f"")
    print(f"VERDICT: the HM measure equals the target 2/9 (selected against a hardcoded")
    print(f"  target; NOT a forced derivation — the VALUE 2/9 is derived target-free in")
    print(f"  predictions/delta_Koide_derivation.py, but this scan is not that derivation).")

...
graph LR
    ...
    q --> pdg["all 12 match measurement<br/>at within ~1σ"]
    l --> pdg
    c --> pdg
```
...
All 12 are read off the **same** resolvent. Varying the spectral channel (which projection of the resolvent), not varying parameters. If the resolvent were the wrong object, **all 12 would fail together**; the fact that all 12 hit measurement at within about 1σ simultaneously is the over-determination.
...
**The day the mass sector is forced into the same resolvent that already governs the flavor and oblique sectors, and it agrees — the program is done.** A universal mass operator already exists (the twelve-fermion operator on the bipartite cover, [Chapter 3](03-the-cover-that-holds-chirality.md)); the next milestone is showing that its outputs cross-validate against the gauge sector through the same single resolvent, without new inputs.

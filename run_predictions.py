...
SECTORS = [
    ("Standard Model — Gauge couplings", [
        ...
        ("g_2",      "g_2",          0.65177,         0.0001,  "(dimensionless)", "SU(2)_L RG from α_GUT. Target is the SCHEME-CONSISTENT g_2=√(4π·α_EM/sin²θ_W)=0.65177 from the same PDG α_EM,sin²θ siblings (g_2 is NOT independent); prior 0.6520 was scheme-inconsistent and gave a spurious −2.52σ (fixed 2026-06-25, verified)."),
    ...
    ("Standard Model — Charged fermion masses", [
        ...
        ("m_b",      "m_b",          4.18,            0.03,    "GeV",  "THEOREM-GRADE-STRUCTURAL-CONDITIONAL (Row P39) + unified channel-read dark Σ=α₁/h at the Perron channel ×(1−α₁/h_P), power 1 (L>0): +2.99σ → +0.22σ. Magnitude theorem-grade (1e-12 cross-check); DOWN sign EMPIRICALLY forced (m_b prefers DOWN +0.22σ vs +5.8σ for UP; consistent across sector), formal sign-derivation open. theorem_dark_self_energy_unified_2026-06-28 §3; heavy_quark_anchor_dark.py."),
        ("m_t",      "m_t",          172.69,          0.30,    "GeV",  "Type-II saturation y_t(GUT)=1 (Row P38) + unified channel-read dark ×(1−α₁/h_P²), power 2 (L=0 saturation): +4.71σ → −0.95σ. Magnitude theorem-grade; DOWN sign EMPIRICALLY forced (cross-checked, consistent), formal sign-derivation open. theorem_dark_self_energy_unified_2026-06-28 §3; heavy_quark_anchor_dark.py."),
        ("m_e",      "m_e",          0.51099895e-3,   1.5e-13, "GeV",  "OPEN MISS (−70 ppm, un-derived). The m_e/m_τ ratio misses by −0.0083%. δ=2/9 is the FORCED leading Koide phase (φ·s, build_dN); the residual is the OPEN subleading per-rep O(α₁³) dark Dyson diagram (conjecture-grade MDL; all 11 operator-routes explored). CONTEXT (not a verdict): in the Koide-phase variable the gap is 0.53σ and m_e is exp-sensitive to δ (σ_mass~10⁻⁹) so the mass-σ is hypersensitivity-amplified — but per the TOP-DOWN LAW the 0.53σ phase consistency does NOT make the mass fine; the miss STAYS OPEN (not a pass/floor/artifact). Koide ADOPTED in the live file. Scale cond. G1."),
        ("m_μ",      "m_mu",         0.1056583755,    2.3e-9,  "GeV",  "OPEN MISS (−60.5 ppm, un-derived). The m_μ/m_τ ratio misses by −0.007% — the sibling of m_e's −70 ppm, the same OPEN subleading per-rep O(α₁³) dark Dyson diagram (conjecture-grade MDL). CONTEXT (not a verdict): 0.53σ in the Koide-phase variable, hypersensitivity-amplified in mass — but the phase consistency does NOT make the mass fine; the miss STAYS OPEN. Koide ADOPTED. Scale cond. G1."),
    ...
    ("Framework-internal", [
        ...
        ("tan β",    "tan_beta",     None,            None,    "(dimensionless)", "MSSM ratio of Higgs VEVs. tan β ≈ 60.07 (live chain predictions/tan_beta.py). **Row P46 RECONCILED 2026-06-16:** the prior 60.07-vs-44.73 dispute was a SCALE-PLACEMENT difference, not a bug — this chain places the framework Type-III/IV Yukawas at the LOW scale (M_Z; correct per the walker-length §11 scale-assignment rule, since y_τ=0.007226 gives the physical m_τ=v·y_τ=1.777 GeV) and runs up to GJ y_b/y_τ=k*=3 at GUT; proofs/masses/srs_tan_beta.py placed them at M_GUT (→44.73) and is superseded. tan β is BNDY-EXT (external MSSM RGE, large-tan-β regime is RGE/threshold-sensitive; not load-bearing for m_t/m_b; not directly observed). Invariant framework content = GJ = y_b/y_τ = k* = 3 (theorem-grade)."),

...
# --- PREDICTED VALUE -----------------------------------------
# Value:       125.578 GeV  (computed below)
# Deviation:   +0.378 GeV absolute, +0.302% relative, +3.43 σ_PDG.
#
# Clause 8 is evaluated against σ_PDG only. The +3.43σ_PDG residual corresponds
# to the un-derived Feshbach analog on λ — v matches v_obs essentially
# exactly via the G_F round-trip in N_hub, so the entire residual lives on λ.
...
print("  NOT YET derived for λ: Feshbach analog open (Priority 4.4 step 2.1;")
print("    an internal working note). The +3.43σ_PDG")
print("    residual on m_H lives entirely on λ (v matches v_obs essentially")
print("    exactly via the G_F round-trip) and corresponds to this missing")
print("    analog.")
print()

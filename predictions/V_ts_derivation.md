# Derivation of $|V_{ts}|$ — THEOREM-GRADE-NUMERICAL via CKM unitarity (Type-4 inheritance)

**Status (created 2026-05-02 via parameter_linter combined cleanup walk):** THEOREM-GRADE-NUMERICAL via Type-4 inheritance from Rows P3, P4, P14, P15. Labeling layer is OTHER-SMUGGLE residue inherited from Row P14, NON-BLOCKING for predictive content per the (Z/2)³ Angle D verdict (commit `e5ef667`). Numerical match: -1.50σ on PDG 2024.
...
The CKM matrix element $|V_{ts}|$ is observed at $0.0411 \pm 0.00083$ (PDG 2024 (B_s mixing + unitarity)). The framework predicts $|V_{ts}| = 0.039852$ at $-1.50\sigma$ via Type-4 inheritance from the four independent framework-derived inputs
...
$$|V_{ts}| = \big| -c_{12} s_{23} - s_{12} c_{23} s_{13} e^{i\delta} \big|.$$
...
CAS-verified by `predictions/V_ts.py` (assertion: implementation matches pure function to $10^{-12}$).
...
$$\boxed{|V_{ts}| = 0.039852.}$$

---

## Comparison with experiment

- PDG 2024 (B_s mixing + unitarity): $|V_{ts}| = 0.0411 \pm 0.00083$.
- Framework prediction: $|V_{ts}| = 0.039852$.
- Deviation: $-1.50\sigma$.
...
The four amplitudes that combine into $|V_{ts}|$ each carry an OTHER-SMUGGLE residue: which (i,j) pair gets which framework amplitude is fixed by empirical anchoring of names rather than by structural derivation. The (Z/2)³ Angle D verdict (2026-04-30) verifies all 77 prediction values are invariant under PS-spinor-weight relabeling; only (PDG name → value) pairings shift. Therefore the labeling residue does not affect $|V_{ts}|$'s numerical value — it is a global naming convention pinned empirically.
...
- **Clause 8 (numerical match):** SOFT-TENSION (within 3σ; uncertainty derived via unitarity assumption). Systematic floor: zero — $|V_{ts}|$ is a "pure" structural prediction per Clause 8(b).

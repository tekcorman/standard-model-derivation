#!/usr/bin/env python3
"""
Canonical prediction file for V_tb (CKM matrix element, |V_tb|).
...
Clause 8 SOFT-TENSION (within 3σ; PDG uncertainty is correlated via unitarity).

Audit anchor: Row P14 V_ub family (M1c) of `docs/parameters/parameter_uniqueness_ledger.md`.

    V_tb = c_23·c_13   (Chau-Keung 1984 / PDG standard parameterization)
...
History:
  - Pre-2026-05-02: no prediction file; V_tb tracked only via the
  ...
  - 2026-05-02: Type-4 closure shipped via parameter_linter combined cleanup
    walk. Numerical value vs PDG = +1.62σ.
"""

# ============================================================
# PARAMETER: V_tb (CKM matrix element, |V_tb|)
# ============================================================

# --- OBSERVED VALUE ------------------------------------------
# Value:       |V_tb| = 0.999118 ± 3.1e-05
# Source:      PDG 2024 Review of Particle Physics, CKM review (single-top + unitarity).
# PDG edition: 2024.

# --- PREDICTED VALUE -----------------------------------------
# Value:       |V_tb| (CKM unitarity from V_us, V_cb, V_ub, δ_CP)
# Deviation:   +1.62σ (see runtime output for exact)
# Status:      THEOREM-GRADE-NUMERICAL (Type-4 inheritance);
#              labeling data-anchored / non-blocking via Row P14.
#              Systematic floor: zero. Clause 8: SOFT-TENSION (within 3σ; PDG uncertainty is correlated via unitarity).
...
V_tb_obs = 0.999118
V_tb_unc = 3.1e-05
dev_abs = V_tb - V_tb_obs
dev_rel = dev_abs / V_tb_obs
dev_sigma = dev_abs / V_tb_unc

print("=" * 68)
print("  V_tb  --  THEOREM-GRADE-NUMERICAL via CKM unitarity")
print("=" * 68)
...
print()
print(f"  V_tb = c_23·c_13")
print(f"        = {V_tb:.10f}")
print()
print(f"  PDG 2024  : {V_tb_obs} ± {V_tb_unc}")
...
print("    Step 3 [Type 2]: V_tb closed-form")


# --- PURE FUNCTION -------------------------------------------

@functools.lru_cache(maxsize=None)
def predict_V_tb(V_us, V_cb, V_ub, cos_delta_CP):
    """
    Compute |V_tb| from the unitary CKM matrix in standard parameterization.

    Formula: V_tb = c_23·c_13.
    ...
    float
        Predicted |V_tb|.
    """
    return build_ckm_magnitudes(V_us, V_cb, V_ub, cos_delta_CP).V_tb
...
if __name__ == "__main__":
    ...
    print(f"    V_tb = {pure_result:.6f}  (PDG: {V_tb_obs} ± {V_tb_unc}, {dev_sigma:+.2f}σ)")

...
STATUS UPDATE (2026-06-29, dark-sector unification): this exponent α₁=((k-1)/k)^(g-2) is the
MAGNITUDE of the ONE unified dark self-energy Σ = α₁/h read across the entire precision sector.
ORGANIZING SOURCE: docs/theorems/theorem_dark_self_energy_unified_2026-06-28.md — one resolvent
G_NB, read at the Perron channel (heavy quarks/gauge) or shell (leptons/ν), projected by the
coupling (c=1 channel read / c_S=1/12 gauge-singlet / c_F=α₁² vertex), power by the L-rule. The
"Family A/B/C/D" taxonomy is this one read at different channels × projections.
  • MAGNITUDE (this file): THEOREM-GRADE, cross-checked 1e-12 on the lepton/ν coefficients (√3/4, √5/4).
  • SIGN: the channel-read DOWN sign is DERIVED conditional on the mass=recurrence-rate foundation (the rate
    reading → DOWN) + empirically cross-checked (m_b prefers DOWN +0.22σ vs +5.8σ; consistent across the sector);
    the ONLY open piece is the standalone CAS lemma formalizing the rate-reading selection (§3: the foundation read
    generically admits no-change/DOWN/UP; the RATE definition picks DOWN — settled DOWN, not undetermined).
    The VERTEX sign (y_τ c_F) IS formally forced (closed-fermion-loop −1, Peskin §4.8).
Per-prediction map + the M_Z bracket: an internal working note,
docs/theorems/theorem_M_Z_shell_vertexbox_closure_2026-06-29.md.
"""

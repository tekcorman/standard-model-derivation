# Derivation of $|V_{ud}|$ — THEOREM-GRADE-NUMERICAL via CKM unitarity (Type-4 inheritance)

**Status (created 2026-05-02 via parameter_linter combined cleanup walk):** THEOREM-GRADE-NUMERICAL via Type-4 inheritance from Rows P3, P4, P14, P15. Labeling layer is OTHER-SMUGGLE residue inherited from Row P14, NON-BLOCKING for predictive content per the (Z/2)³ Angle D verdict (commit `e5ef667`). Numerical match: +0.01σ on PDG 2024.
...
The CKM matrix element $|V_{ud}|$ is observed at $0.97435 \pm 0.00016$ (PDG 2024 (super-allowed 0+ → 0+ nuclear beta decays)). The framework predicts $|V_{ud}| = 0.974351$ at $+0.01\sigma$ via Type-4 inheritance from the four independent framework-derived inputs
...
CAS-verified by `predictions/V_ud.py` (assertion: implementation matches pure function to $10^{-12}$).
...
$$\boxed{|V_{ud}| = 0.974351.}$$

---

## Comparison with experiment

- PDG 2024 (super-allowed 0+ → 0+ nuclear beta decays): $|V_{ud}| = 0.97435 \pm 0.00016$.
- Framework prediction: $|V_{ud}| = 0.974351$.
- Deviation: $+0.01\sigma$.
...
The four amplitudes that combine into $|V_{ud}|$ each carry an OTHER-SMUGGLE residue: which (i,j) pair gets which framework amplitude is fixed by empirical anchoring of names rather than by structural derivation. The (Z/2)³ Angle D verdict (2026-04-30) verifies all 77 prediction values are invariant under PS-spinor-weight relabeling; only (PDG name → value) pairings shift. Therefore the labeling residue does not affect $|V_{ud}|$'s numerical value — it is a global naming convention pinned empirically.
...
- **Clause 8 (numerical match):** PASS. Systematic floor: zero — $|V_{ud}|$ is a "pure" structural prediction per Clause 8(b).

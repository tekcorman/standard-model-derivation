# Derivation of $|V_{cd}|$ — THEOREM-GRADE-NUMERICAL via CKM unitarity (Type-4 inheritance)
...
The CKM matrix element $|V_{cd}|$ is observed at $0.22487 \pm 0.00068$ (PDG 2024 (charm semileptonic)). The framework predicts $|V_{cd}| = 0.224866$ at $-0.01\sigma$ via Type-4 inheritance from the four independent framework-derived inputs
...
$$|V_{cd}| = \big| -s_{12} c_{23} - c_{12} s_{23} s_{13} e^{i\delta} \big|.$$
...
CAS-verified by `predictions/V_cd.py` (assertion: implementation matches pure function to $10^{-12}$).
...
$$\boxed{|V_{cd}| = 0.224866.}$$

---

## Comparison with experiment

- PDG 2024 (charm semileptonic): $|V_{cd}| = 0.22487 \pm 0.00068$.
- Framework prediction: $|V_{cd}| = 0.224866$.
...
The four amplitudes that combine into $|V_{cd}|$ each carry an OTHER-SMUGGLE residue: which (i,j) pair gets which framework amplitude is fixed by empirical anchoring of names rather than by structural derivation. The (Z/2)³ Angle D verdict (2026-04-30) verifies all 77 prediction values are invariant under PS-spinor-weight relabeling; only (PDG name → value) pairings shift. Therefore the labeling residue does not affect $|V_{cd}|$'s numerical value — it is a global naming convention pinned empirically.
...
- **Clause 8 (numerical match):** PASS. Systematic floor: zero — $|V_{cd}|$ is a "pure" structural prediction per Clause 8(b).

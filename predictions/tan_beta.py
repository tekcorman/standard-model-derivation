...
LIVE PREDICTION
===============
    tan(β) ≈ 44.73   [framework-internal MSSM RGE + GJ self-consistency]

Cross-reference: `proofs/masses/srs_tan_beta.py` computes this value via
the same chain (with observed gauge couplings instead of framework; ~0.5%
...
# --- PREDICTED VALUE -----------------------------------------
# Value:       tan(β) ≈ 44.73 (via framework MSSM RGE with GJ=3)
...
def _solve_tan_beta():
    ...
    `except: return 44.73` was masking this disagreement. The live chain
    surfaces honestly here; reconciliation between the documented proof and
    the live chain is open work (see parameter_uniqueness_ledger.md Row P46).
    """
    target = _k  # GJ = k* = 3

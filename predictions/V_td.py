#!/usr/bin/env python3
"""
Canonical prediction file for V_td (CKM matrix element, |V_td|).
...
Audit anchor: Row P14 V_ub family (M1c) of `docs/parameters/parameter_uniqueness_ledger.md`.
    ...
    V_td = |s_12·s_23 − c_12·c_23·s_13·e^(iδ)|   (Chau-Keung 1984 / PDG standard parameterization)
...
History:
  - Pre-2026-05-02: no prediction file; V_td tracked only via the
  ...
  - 2026-05-02: Type-4 closure shipped via parameter_linter combined cleanup
    walk. Numerical value vs PDG = +0.42σ.
"""

# ============================================================
# PARAMETER: V_td (CKM matrix element, |V_td|)
# ============================================================

# --- OBSERVED VALUE ------------------------------------------
# Value:       |V_td| = 0.00854 ± 0.00023
# Source:      PDG 2024 Review of Particle Physics, CKM review (B_d mixing).
# PDG edition: 2024.

# --- PREDICTED VALUE -----------------------------------------
# Value:       |V_td| (CKM unitarity from V_us, V_cb, V_ub, δ_CP)
# Deviation:   +0.42σ (see runtime output for exact)
...
V_td_obs = 0.00854
V_td_unc = 0.00023
dev_abs = V_td - V_td_obs
dev_rel = dev_abs / V_td_obs
dev_sigma = dev_abs / V_td_unc

print("=" * 68)
print("  V_td  --  THEOREM-GRADE-NUMERICAL via CKM unitarity")
print("=" * 68)
...
print()
print(f"  V_td = |s_12·s_23 − c_12·c_23·s_13·e^(iδ)|")
print(f"        = {V_td:.10f}")
print()
print(f"  PDG 2024  : {V_td_obs} ± {V_td_unc}")
...
print("    Step 3 [Type 2]: V_td closed-form")


# --- PURE FUNCTION -------------------------------------------

@functools.lru_cache(maxsize=None)
def predict_V_td(V_us, V_cb, V_ub, cos_delta_CP):
    """
    Compute |V_td| from the unitary CKM matrix in standard parameterization.

    Formula: V_td = |s_12·s_23 − c_12·c_23·s_13·e^(iδ)|.
    ...
    float
        Predicted |V_td|.
    """
    return build_ckm_magnitudes(V_us, V_cb, V_ub, cos_delta_CP).V_td
...
if __name__ == "__main__":
    ...
    print(f"    V_td = {pure_result:.6f}  (PDG: {V_td_obs} ± {V_td_unc}, {dev_sigma:+.2f}σ)")

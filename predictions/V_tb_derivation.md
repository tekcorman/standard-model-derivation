# Derivation of $|V_{tb}|$ — THEOREM-GRADE-NUMERICAL via CKM unitarity (Type-4 inheritance)

**Status (created 2026-05-02 via parameter_linter combined cleanup walk):** THEOREM-GRADE-NUMERICAL via Type-4 inheritance from Rows P3, P4, P14, P15. Labeling layer is OTHER-SMUGGLE residue inherited from Row P14, NON-BLOCKING for predictive content per the (Z/2)³ Angle D verdict (commit `e5ef667`). Numerical match: +1.62σ on PDG 2024.
...
The CKM matrix element $|V_{tb}|$ is observed at $0.999118 \pm 3.1e-05$ (PDG 2024 (single-top + unitarity)). The framework predicts $|V_{tb}| = 0.999168$ at $+1.62\sigma$ via Type-4 inheritance from the four independent framework-derived inputs
...
CAS-verified by `predictions/V_tb.py` (assertion: implementation matches pure function to $10^{-12}$).
...
$$\boxed{|V_{tb}| = 0.999168.}$$

---

## Comparison with experiment

- PDG 2024 (single-top + unitarity): $|V_{tb}| = 0.999118 \pm 3.1e-05$.
- Framework prediction: $|V_{tb}| = 0.999168$.
- Deviation: $+1.62\sigma$.
...
The four amplitudes that combine into $|V_{tb}|$ each carry an OTHER-SMUGGLE residue: which (i,j) pair gets which framework amplitude is fixed by empirical anchoring of names rather than by structural derivation. The (Z/2)³ Angle D verdict (2026-04-30) verifies all 77 prediction values are invariant under PS-spinor-weight relabeling; only (PDG name → value) pairings shift. Therefore the labeling residue does not affect $|V_{tb}|$'s numerical value — it is a global naming convention pinned empirically.
...
- **Clause 8 (numerical match):** SOFT-TENSION (within 3σ; PDG uncertainty derived from unitarity assumption — correlated). Systematic floor: zero — $|V_{tb}|$ is a "pure" structural prediction per Clause 8(b).

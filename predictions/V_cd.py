#!/usr/bin/env python3
"""
Canonical prediction file for V_cd (CKM matrix element, |V_cd|).
...
Audit anchor: Row P14 V_ub family (M1c) of `docs/parameters/parameter_uniqueness_ledger.md`.
    ...
    V_cd = |−s_12·c_23 − c_12·s_23·s_13·e^(iδ)|   (Chau-Keung 1984 / PDG standard parameterization)
...
History:
  - Pre-2026-05-02: no prediction file; V_cd tracked only via the
  ...
  - 2026-05-02: Type-4 closure shipped via parameter_linter combined cleanup
    walk. Numerical value vs PDG = -0.01σ.
"""

# ============================================================
# PARAMETER: V_cd (CKM matrix element, |V_cd|)
# ============================================================

# --- OBSERVED VALUE ------------------------------------------
# Value:       |V_cd| = 0.22487 ± 0.00068
# Source:      PDG 2024 Review of Particle Physics, CKM review (charm semileptonic).
# PDG edition: 2024.

# --- PREDICTED VALUE -----------------------------------------
# Value:       |V_cd| (CKM unitarity from V_us, V_cb, V_ub, δ_CP)
# Deviation:   -0.01σ (see runtime output for exact)
...
V_cd_obs = 0.22487
V_cd_unc = 0.00068
dev_abs = V_cd - V_cd_obs
dev_rel = dev_abs / V_cd_obs
dev_sigma = dev_abs / V_cd_unc

print("=" * 68)
print("  V_cd  --  THEOREM-GRADE-NUMERICAL via CKM unitarity")
print("=" * 68)
...
print()
print(f"  V_cd = |−s_12·c_23 − c_12·s_23·s_13·e^(iδ)|")
print(f"        = {V_cd:.10f}")
print()
print(f"  PDG 2024  : {V_cd_obs} ± {V_cd_unc}")
...
print("    Step 3 [Type 2]: V_cd closed-form")


# --- PURE FUNCTION -------------------------------------------

@functools.lru_cache(maxsize=None)
def predict_V_cd(V_us, V_cb, V_ub, cos_delta_CP):
    """
    Compute |V_cd| from the unitary CKM matrix in standard parameterization.

    Formula: V_cd = |−s_12·c_23 − c_12·s_23·s_13·e^(iδ)|.
    ...
    float
        Predicted |V_cd|.
    """
    return build_ckm_magnitudes(V_us, V_cb, V_ub, cos_delta_CP).V_cd
...
if __name__ == "__main__":
    ...
    print(f"    V_cd = {pure_result:.6f}  (PDG: {V_cd_obs} ± {V_cd_unc}, {dev_sigma:+.2f}σ)")

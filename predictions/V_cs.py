#!/usr/bin/env python3
"""
Canonical prediction file for V_cs (CKM matrix element, |V_cs|).
...
Audit anchor: Row P14 V_ub family (M1c) of `docs/parameters/parameter_uniqueness_ledger.md`.
    ...
    V_cs = |c_12·c_23 − s_12·s_23·s_13·e^(iδ)|   (Chau-Keung 1984 / PDG standard parameterization)
...
History:
  - Pre-2026-05-02: no prediction file; V_cs tracked only via the
  ...
  - 2026-05-02: Type-4 closure shipped via parameter_linter combined cleanup
    walk. Numerical value vs PDG = +0.33σ.
"""

# ============================================================
# PARAMETER: V_cs (CKM matrix element, |V_cs|)
# ============================================================

# --- OBSERVED VALUE ------------------------------------------
# Value:       |V_cs| = 0.97349 ± 0.00016
# Source:      PDG 2024 Review of Particle Physics, CKM review (charm semileptonic).
# PDG edition: 2024.

# --- PREDICTED VALUE -----------------------------------------
# Value:       |V_cs| (CKM unitarity from V_us, V_cb, V_ub, δ_CP)
# Deviation:   +0.33σ (see runtime output for exact)
...
V_cs_obs = 0.97349
V_cs_unc = 0.00016
dev_abs = V_cs - V_cs_obs
dev_rel = dev_abs / V_cs_obs
dev_sigma = dev_abs / V_cs_unc

print("=" * 68)
print("  V_cs  --  THEOREM-GRADE-NUMERICAL via CKM unitarity")
print("=" * 68)
...
print()
print(f"  V_cs = |c_12·c_23 − s_12·s_23·s_13·e^(iδ)|")
print(f"        = {V_cs:.10f}")
print()
print(f"  PDG 2024  : {V_cs_obs} ± {V_cs_unc}")
...
print("    Step 3 [Type 2]: V_cs closed-form")


# --- PURE FUNCTION -------------------------------------------

@functools.lru_cache(maxsize=None)
def predict_V_cs(V_us, V_cb, V_ub, cos_delta_CP):
    """
    Compute |V_cs| from the unitary CKM matrix in standard parameterization.

    Formula: V_cs = |c_12·c_23 − s_12·s_23·s_13·e^(iδ)|.
    ...
    float
        Predicted |V_cs|.
    """
    return build_ckm_magnitudes(V_us, V_cb, V_ub, cos_delta_CP).V_cs
...
if __name__ == "__main__":
    ...
    print(f"    V_cs = {pure_result:.6f}  (PDG: {V_cs_obs} ± {V_cs_unc}, {dev_sigma:+.2f}σ)")

...
STATUS: UNIQUE — THEOREM-GRADE for the FORM/FORMULA only. ⚠ The VALUE is a
G_F-calibration ROUND-TRIP — N_hub is pinned from the measured G_F so that
v_pred = v_obs BY CONSTRUCTION; the −0.0001%/0σ is a calibration artifact, not a
prediction (see the PREDICTED VALUE note below, added 2026-06-30).
...
v_Higgs graduates (for the FORM/FORMULA; the value is the G_F round-trip above) from
"STRICT-SOLID conditional on G1" to **UNIQUE THEOREM-GRADE**
...
# --- PREDICTED VALUE -----------------------------------------
# Value:       v = 246.22 GeV (live; v_pred = v_obs to -0.0001%).
#
# GRADE SCOPE (2026-06-30 audit — an internal working note):
#   UNIQUE-THEOREM-GRADE applies to the FORMULA / FORM — the structure
#   v = δ²·M_P·dark/(√2·N^{1/4}) and its forced skeleton (the 1/√2 screw overlap,
#   the N^{-1/4} finite-size exponent, the 5/12 count) — NOT to the numerical VALUE.
#   The VALUE is a CALIBRATION ROUND-TRIP: N_hub is pinned from the measured G_F so
#   that v_pred = v_obs BY CONSTRUCTION; the -0.0001% / 0σ is a calibration artifact,
#   not a prediction. (The prior "245.68 GeV / -0.22% / -4.5σ" block was STALE and
#   self-contradictory with the live G_F-pinned output — removed 2026-06-30.)
#   Open sub-gaps inside the form (honest): the δ² normalization (CANDIDATE, open
#   coupling-normalization gap) and the 5/12 denominator/winding conventions.
#   The genuine PREDICTIONS from N_hub are the cosmology (H_0, t_0, Λ) — not v.
#   m_H = √(2λ)v inherits the calibrated v; any m_H residual lives on λ (see m_H.py).
...
#
# ⚠ DISPUTED / LIKELY DOES NOT BELONG (2026-06-30 audit, an internal working note):
#   Family-D is a legged scattering-vertex object (fermion/Higgs legs, JW −1 loop sign); v is a
#   one-point CONDENSATE ⟨H⟩, not a vertex — the "(1H+0F) v vertex" is a category stretch. The term
#   is NOT in the live formula (comment-only), and its "absorption check" is a by-design tautology
#   (any factor on v is absorbable since N_hub is pinned to v_obs). Net: it should NOT be applied to v;
#   omitting it is correct, and H_0 = +1.56σ STANDS (NOT the +2.39σ that applying it would give).
# Family D sub-leading on v (2026-05-15 sweep item 5) [retained for record; see ⚠ above]:
